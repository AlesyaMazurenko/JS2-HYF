...
const shortNames = movies.filter((movie)=> movie.title.length < 4);
console.log('movies with short title', shortNames);

// 2.Create an array of movie titles with long movie titles
const lll = [];
const longMovies = movies.filter((movie)=>movie.title.length > 60);
const longNames = longMovies.map((movie) => movie.title);
console.log('long Names', longNames);

// 3.Count the number of movies made between 1980-1989 (including both the years)

const yearsFilteredMovie = movies.filter(filterByYear).length;

function filterByYear(movies) {
  return (movies.year) >= 1980 && movies.year<=1989;
}
console.log(" movies made 1980-1989", yearsFilteredMovie);
...
console.log('New array adding tag', addtag);
...
const higterRating = movies.filter(filterByRating).map((movie) => movie.title)

function filterByRating(movies){
  return movies.rating > 6;
}

console.log('Movies with Rating over 6', higterRating);
...
function countKeyWords(movies) {
  let counter = 0;
  const keyWords = ["Surfer", "Alien", "Benjamin"];
  for (let i = 0; i < keyWords.length; i++) {
    movies.filter(movie => {
      if (movie.title.includes(keyWords[i])) {
        counter += 1;
      }
    })
  }
...
console.log('counter', countKeyWords(movies));
...
function duplicationInNames() {
  const newArr = movies.filter((movie) => {
    const nameMovie = movie.title.split(' ');
    if (nameMovie.length > 1) {
      for (let i = 0; i < nameMovie.length; i++) {
        for (let j = i+1; j < nameMovie.length; j++) {
          if (nameMovie[i] === nameMovie[j]) {
            return movie.title; 
          }
        }
      }
...
console.log('Duplication in titles', duplicationInNames())

// 8. Calculate the average rating of all the movies using reduce. Optional

const initialValue = 0;
const averageRating = (movies.reduce((accum, movie)=> accum+movie.rating, initialValue,
)) / movies.length;
...
function countByTag() {
  const goodCount = addtag.filter((movie) => movie.tag === 'Good').length;
  const averageCount = addtag.filter((movie) => movie.tag === "Average").length;
  const badCount = addtag.filter((movie) => movie.tag === "Bad").length;
  const result={'goodMovies': goodCount, 
  'averageMovies': averageCount, 
  'badMovies': badCount}
  console.log(result);
}
